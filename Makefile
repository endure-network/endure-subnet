...
lint: ## Run ruff check + format check
	$(PYTHON) -m ruff check endure/ neurons/ scripts/ tests/
	$(PYTHON) -m ruff format --check endure/ neurons/ scripts/ tests/

format: ## Apply ruff format + fix
	$(PYTHON) -m ruff check --fix endure/ neurons/ scripts/ tests/
	$(PYTHON) -m ruff format endure/ neurons/ scripts/ tests/
...
devnet-cycle: ## Run Alpha Risk R5 compressed full cycle against an already-running local subtensor
	$(PYTHON) scripts/run_devnet_cycle.py --netuid $${NETUID:?set NETUID from scripts/dev/seed_chain.sh} --network $${NETWORK:-ws://127.0.0.1:9946}

devnet-fault-miner: ## Restart the Alpha miner after its commit and require full recovery
	$(PYTHON) scripts/run_devnet_cycle.py --netuid $${NETUID:?set NETUID from scripts/dev/seed_chain.sh} --network $${NETWORK:-ws://127.0.0.1:9946} --fault miner-restart-after-commit --round-seconds 240 --timeout-seconds 720

devnet-fault-validator: ## Restart the Alpha validator after accepting a commit and require full recovery
	$(PYTHON) scripts/run_devnet_cycle.py --netuid $${NETUID:?set NETUID from scripts/dev/seed_chain.sh} --network $${NETWORK:-ws://127.0.0.1:9946} --fault validator-restart-after-commit --round-seconds 240 --timeout-seconds 720

devnet-fault-miner-state-loss: ## Wipe the miner's state after its commit and require the lost round to be surfaced
	$(PYTHON) scripts/run_devnet_cycle.py --netuid $${NETUID:?set NETUID from scripts/dev/seed_chain.sh} --network $${NETWORK:-ws://127.0.0.1:9946} --fault miner-state-loss-after-commit --round-seconds 240 --timeout-seconds 720
